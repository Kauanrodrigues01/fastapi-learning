...
def test_get_token_with_invalid_email(client, user):
    response = client.post(
        '/token',
        data={'username': 'invalid_email@example.com', 'password': user.clean_password}
    )
    json_response = response.json()
...
def test_update_user_success(client, user, session, token):
    user_data = {
...
def test_updated_user_without_token(client, token):
    user_data = {
        'username': 'test_updated_name',
        'email': 'teste_updated@example.com',
...
def test_update_user_with_duplicate_username(client, user, session, token):
    # Criando um segundo usuário
    user2 = User(username='user2', email='user2@example.com', password='user2password')
    session.add(user2)
    session.commit()
...
def test_update_user_with_duplicate_email(client, user, session, token):
    # Criando um segundo usuário
    user2 = User(username='user2', email='user2@example.com', password='user2password')
    session.add(user2)
    session.commit()
...
def test_delete_user_success(client, user, session, token):
