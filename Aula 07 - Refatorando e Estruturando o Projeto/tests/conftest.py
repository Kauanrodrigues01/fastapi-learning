...
def user(session: Session):
    ...
    if user:  # Evitar duplicações de usuarios nos testes
        return user

    password = 'testtest'
    user = User(username='Test', email='test@test.com', password=get_password_hash(password))
    session.add(user)
    session.commit()
...
@pytest.fixture
def token(user):
    return create_access_token(data_payload={'sub': user.email})
